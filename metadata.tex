...
\hypersetup{
  ...
  filecolor=magenta,
  urlcolor=cyan,
  pdftitle={Rome Total War\: The Boardgame Rule Book},
  pdfpagemode=UseNone,
}
...
\def\fonts{\_assets/fonts}

\def\repourl{https://github.com/fmestevez/rtw-bg-unofficial-rulebook}
\def\bggthreadurl{https://boardgamegeek.com/thread/3426675/rulebook-rewrite-project}
...
\newcommand{\addsection}[3][1]{
  \vspace*{-5em} % Adjust vertical positioning
  \hspace*{-1em}
  \makebox[0pt][l]{
    \raisebox{-\totalheight}[0pt][0pt]{
      \begin{tikzpicture}
        % Insert the banner image
        \node[anchor=north] (banner) at (0, 0) {\includegraphics[width=\columnwidth, keepaspectratio]{\layout/section_heading.png}};
        % Place the section title inside the banner
        \node[align=center, text=white, font=\fontsize{16}{16}\selectfont\bfseries] at ([yshift=-1cm] banner.center) {#2};
      \end{tikzpicture}
    }
  }
  % Reserve space below the banner to prevent overlap with body text
  \vspace{12em} % Adjust spacing to push body text further down
  \begin{fullwidth}[outermargin=0.16\textwidth, innermargin=0.16\textwidth]
    \begin{center}
...
\AddToHook{shipout/background}{%
  \iftoggle{noartbackground}{}{
    \put (0in,-\paperheight){\includegraphics[width=\paperwidth,height=\paperheight]{\layout/content.jpg}}
  }
  % \iftoggle{printable}{
...
\ifdefvoid{\translatedauthor}{
  \author{\textlang{english}{Facundo Estevez}}
}{
